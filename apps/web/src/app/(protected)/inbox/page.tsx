...
function OfferRow({ offer, role }: { offer: Offer; role: Tab }) {
  const image = offer.listing?.images[0]?.imageUrl
  const otherParty = role === 'buyer' ? offer.seller : offer.buyer

  return (
    <Link href={`/offers/${offer.id}`} className="flex items-center gap-3 border-b border-beige-200 px-4 py-3.5 hover:bg-beige-100 transition-colors last:border-b-0">
      <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-beige-200">
        {image ? (
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        <span className="text-sm font-semibold text-stone-800">${offer.price.toFixed(2)}</span>
    ...
    return (
      <div className="py-16 text-center">
        <p className="text-sm text-stone-400">
          {role === 'buyer' ? 'No offers sent yet.' : 'No offers received yet.'}
        </p>
        {role === 'buyer' && (
  ...
  return (
    <div className="card overflow-hidden">
      {data.items.map(offer => (
        <OfferRow key={offer.id} offer={offer} role={role} />
      ))}
    </div>
  )
}
        ...
        {isSeller && (
          <div className="mb-4 flex rounded-lg border border-taupe-300 bg-white p-1">
            {(['seller', 'buyer'] as Tab[]).map(tab => (
              ...
              >
                {tab === 'seller' ? 'Received' : 'Sent'}
              </button>
            ))}
          </div>
        )}

        <OffersList role={activeTab} />
      </div>
    </>
  )
}
